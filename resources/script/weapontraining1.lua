...
WEAPON = {BURST = 2010008, SIGMA = 2000017}
...
function Tutorial_Step_First(step)
  if step == 0 then
    ...
    TutorialSystem2:SetWeaponSlot(ACTOR.ME, 0, WEAPON.BURST, 0)
    TutorialSystem2:SetWeaponSlot(ACTOR.ME, 1, WEAPON.SIGMA, 0)
...
function Tutorial_Loop()
  ...
  if step == step_c then
    ...
    TutorialSystem2:CreateActorPos(ACTOR.ENEMY_1, true, MAKEVECTOR3(POS.ENEMY_SPAWN._01[1], POS.ENEMY_SPAWN._01[2], POS.ENEMY_SPAWN._01[3]), 0, 100, 100, "Tutorial")
  ...
  if step == step_c then
    ...
    if 3 <= TutorialSystem2:GetPlayerAttackCount(Weapon1State) and 3 <= TutorialSystem2:GetDamageCounter(ACTOR.ENEMY_1) then
  ...
  TempCount3 = step_c
  if step == step_c then
    if TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_USE_WEAPON1) == true and TutorialSystem2:GetData(0) == 0 then
      TutorialSystem2:SetData(0, 1)
    end
    if TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_NORMAL) == true and TutorialSystem2:GetData(0) == 1 then
      TutorialSystem2:SetData(0, 2)
    end
    if TutorialSystem2:GetTimerState(0) == TIMER.END then
      TutorialSystem2:LockGamePlayInput(true)
      TutorialSystem2:PushActorCommandState(ACTOR.ME, STATE_NORMAL, 0)
    ...
    if 3 <= TutorialSystem2:GetDamageCounter(ACTOR.ENEMY_1) then
      TutorialSystem2:SetAllAttackLimit(true)
      TutorialSystem2:LockGamePlayInput(true)
      TutorialSystem2:PushActorCommandState(ACTOR.ME, STATE_NORMAL, 0)
      TutorialSystem2:SetPlayCount()
    ...
    if TutorialSystem2:IsActorLocated(ACTOR.ENEMY_1, POS.ENEMY_SPAWN._01[1], POS.ENEMY_SPAWN._01[2], POS.ENEMY_SPAWN._01[3], 200) == false then
  ...
  if step == step_c then
    if TutorialSystem2:GetReturn() == false and TutorialSystem2:GetPlayCount() == 2 then
      TutorialSystem2:PushActorCommandMoveToTarget(POS.ENEMY_SPAWN._01[1], POS.ENEMY_SPAWN._01[2], POS.ENEMY_SPAWN._01[3], -1, ACTOR.ENEMY_1)
  ...
  step_c = step_c + 1
  if step == step_c and TutorialSystem2:GetTimerState(1) == TIMER.END then
    TutorialSystem2:ChangeMessageWindow(MENT.CHARACTER1, 0)
    TutorialSystem2:PushActorCommandState(ACTOR.ME, STATE_NORMAL, 0)
    TutorialSystem2:SetStep(step + 1)
  end
