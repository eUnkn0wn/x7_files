...
function Tutorial_Step_First(step)
  if step == 0 then
    ...
    TutorialSystem2:PushReloadedAmmoInfinite(2040001)
    ...
    TutorialSystem2:SetAIAttackMove(true)
...
function Tutorial_Loop()
  ...
  if step == step_c then
    ...
    TutorialSystem2:SetChoseWeaponSlot(ACTOR.ENEMY_1)
  ...
  if step == step_c then
    ...
    TutorialSystem2:GetDamageCounter(ACTOR.ENEMY_1)
    TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_USE_WEAPON2)
    TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_USE_WEAPON1_WEAK)
    TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_USE_WEAPON1_STRONG)
    TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_USE_WEAPON1_STRONG1)
    TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_USE_WEAPON1_ATTACK2)
    TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_USE_WEAPON2_WEAK)
    TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_USE_WEAPON2_STRONG)
  end
  step_c = step_c + 1
  if step == step_c then
    ...
    if TutorialSystem2:IsDeadActor(ACTOR.ENEMY_1) == true and TutorialSystem2:GetTimerState(0) == TIMER.END then
      TutorialSystem2:SetStep(step + 1)
    end
  end
  step_c = step_c + 1
