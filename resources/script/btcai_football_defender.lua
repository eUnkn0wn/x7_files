...
local BallSearchRange = 5000
function Initialize(BTCAIActor)
  BTCAIActor:SetAttackTypeInfo(0, BTCAIACTOR_ATTACK_MELEE, 500, 100, 100, 1)
  BTCAIActor:SetAttackTypeInfo(1, BTCAIACTOR_ATTACK_SENTRY_GUN, 0, 100, 100, 1)
  BTCAIActor:SetAttackTypeInfo(2, BTCAIACTOR_ATTACK_AIMSHOT, 4000, 100, 100, 1)
end
function Update(timeDelta, BTCAIActor)
  ...
  elseif true == BTCTDHelper:IsBallOwner(BTCAIActor) then
    ...
    if true == BTCTDHelper:FindAttackableEnemyActor(BTCAIActor, FindAttackResult) then
      ...
      if Length <= BallSearchRange then
