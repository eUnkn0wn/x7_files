...
local StealthOnDistance = 5000
local StealthOffMP = 30
local StealthOnMP = 80
function Initialize(BTCAIActor)
  BTCAIActor:SetAttackTypeInfo(0, BTCAIACTOR_ATTACK_MELEE, 500, 100, 100, 1)
  BTCAIActor:SetAttackTypeInfo(1, BTCAIACTOR_ATTACK_RANGE, 1000, 100, 100, 1)
  BTCAIActor:SetAttackTypeInfo(2, BTCAIACTOR_ATTACK_RANGE, 2000, 100, 100, 1)
...
function Update(timeDelta, BTCAIActor)
  ...
  local TargetActor = BTCTDHelper:FindNearestEnemyActor(BTCAIActor, StartPos, -1)
...
function UsableStealth(BTCAIActor, NearestActor)
  ...
  local Length = BTCTDHelper:GetLengthTwoVector(StartPos, EndPos)
  if Length > StealthOnDistance then
    return false
