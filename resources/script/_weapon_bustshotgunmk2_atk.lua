...
function LeftAttack_BustShotGunMK2(OneShotTime, Power, Accuracy, AdditionalPower)
  ...
  STATE:InitLua(260, 40, -40, 250, 650, 650)
  Offset = {
    {0.25, 0.5},
    {0.5, 0.25},
    {0.5, -0.25},
  ...
  for j = 1, 7 do
    ...
    _Attack:InitAttack1(WEAPONTYPE_REVOLVER2, ATTACKATTRIB_BUSTSHOTGUN_MK2_ATTACK, 150, Power * 0.2 + AdditionalPower, AddtionalPower)
    _Attack:InitAttack2(MAKERAY(MAKEVECTOR3(0, 0, 0), MAKEVECTOR3(0, -0.05 * Accuracy, -1), RayDistanceLimit), 1, 1, true)
    local i = 1
    while Offset[i] do
      _Attack:AddCollisionRange(MAKERAY(MAKEVECTOR3(0, 0, 0), MAKEVECTOR3(Offset[i][1] * Accuracy, Offset[i][2] * Accuracy, -1), RayDistanceLimit))
      i = i + 1
    end
  ...
  Attack1:InitAttack1(WEAPONTYPE_REVOLVER2, ATTACKATTRIB_BUSTSHOTGUN_MK2_ATTACK, 150, Power * 0.2 + AdditionalPower, AddtionalPower)
