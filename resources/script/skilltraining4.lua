...
function Init_tutorial2()
  ...
  for i = 1, scriptTotal do
    strKey = string.format("BTC_SKILL_Shield_%d", i)
...
function Tutorial_Step_First(step)
  if step == 0 then
    ...
    TutorialSystem2:CreateActorPos(ACTOR.ME, true, MAKEVECTOR3(POS.MYPOINT._01[1], POS.MYPOINT._01[2], POS.MYPOINT._01[3]), 180, 0, 0, "Danial")
...
function Tutorial_Loop()
  ...
  if step == step_c then
    ...
    TutorialSystem2:SetItemControlUI(SKILL.SHIELD)
  ...
  if step == step_c then
    ...
    if TutorialSystem2:GetTimerState(0) == TIMER.DOING and TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_SKILL_SHIELD) == true then
      TutorialSystem2:SetStep(step + 1)
    end
  end
  step_c = step_c + 1
  if step == step_c then
    ...
    if TutorialSystem2:GetPointMent() == 1 then
      ...
      TutorialSystem2:SKillOff(ACTOR.ME, ONOFF_SHIELD)
  ...
  step_c = step_c + 1
  if step == step_c then
    TutorialSystem2:PushActorCommandAttack(ACTOR.ENEMY_2, ACTOR.ME, true, false, 10000)
    TutorialSystem2:SetTimer(0, 50000)
    TutorialSystem2:SetStep(step + 1)
  end
  step_c = step_c + 1
  if step == step_c then
    if TutorialSystem2:IsDoingState(ACTOR.ME, STATE_SKILL_SHIELD) == true and 0 < TutorialSystem2:ElapsedTimeFromAttack(ACTOR.ME) and TutorialSystem2:ElapsedTimeFromAttack(ACTOR.ME) < 300 then
      TutorialSystem2:SetTimer(1, 2000)
      TutorialSystem2:SetStep(step + 1)
    end
    if TutorialSystem2:GetTimerState(0) == TIMER.END then
  ...
  step_c = step_c + 1
  if step == step_c then
    if TutorialSystem2:GetTimerState(1) == TIMER.END and TutorialSystem2:IsDoingState(ACTOR.ME, STATE_SKILL_SHIELD) == true and 0 < TutorialSystem2:ElapsedTimeFromAttack(ACTOR.ME) and TutorialSystem2:ElapsedTimeFromAttack(ACTOR.ME) < 300 then
      TutorialSystem2:ClearEnableInput()
  ...
  step_c = step_c + 1
  if step == step_c and TutorialSystem2:IsActorLocated(ACTOR.ENEMY_1, TutorialSystem2:GetActorPosX(ACTOR.ME), TutorialSystem2:GetActorPosY(ACTOR.ME), TutorialSystem2:GetActorPosZ(ACTOR.ME), 400) == true then
