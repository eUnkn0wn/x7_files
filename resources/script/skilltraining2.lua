...
function Init_tutorial2()
  ...
  for i = 1, scriptTotal do
    strKey = string.format("BTC_SKILL_Metalic_%d", i)
...
function Tutorial_Step_First(step)
  if step == 0 then
    ...
    TutorialSystem2:SetSkill(ACTOR.ME, SKILL.METALIC)
...
function Tutorial_Loop()
  ...
  if step == step_c then
    ...
    TutorialSystem2:SetItemControlUI(SKILL.METALIC)
  ...
  if step == step_c then
    ...
    if TutorialSystem2:GetTimerState(0) == TIMER.DOING and TutorialSystem2:GetMatchActorState(ACTOR.ME, STATE_SKILL_METALIC) == true then
      TutorialSystem2:SetReturn(false)
      TutorialSystem2:ClearEnableInput()
      TutorialSystem2:SetStep(step + 1)
    end
  ...
  if step == step_c then
    ...
    if TutorialSystem2:GetHP(ACTOR.ENEMY_1) <= 85 then
      TutorialSystem2:ClearEnableInput()
      TutorialSystem2:ClearAllActorCommands(ACTOR.ENEMY_1)
      TutorialSystem2:PushActorCommandState(ACTOR.ENEMY_1, STATE_NORMAL, 0)
      TutorialSystem2:SetStep(step + 1)
